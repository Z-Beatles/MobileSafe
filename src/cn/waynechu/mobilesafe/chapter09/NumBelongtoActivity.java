package cn.waynechu.mobilesafe.chapter09;
...
import cn.waynechu.mobilesafe.chapter09.db.dao.NumBelongtoDao;
...
public class NumBelongtoActivity extends Activity implements OnClickListener {
	...
	private void copyDB(final String dbname) {
		/*
		 * new Thread(){ public void run(){ File file = new File(getFilesDir(),
		 * dbname); if(file.exists()){ Log.i("NumBelongtoActivity", "数据库已存在");
		 * }else{ try { InputStream in = getAssets().open(dbname);
		 * FileOutputStream out = new FileOutputStream(dbname); byte[] buffer =
		 * new byte[1024]; int len = 0; while((len = in.read(buffer))!=-1){
		 * out.write(buffer, 0 ,len); } in.close(); out.close(); } catch
